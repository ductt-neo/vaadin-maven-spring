 ...
 */
public class ScoringRuleCaseDaoImpl
        extends AbstractHibernateDAOImpl<ScoringRuleCase, Integer> implements ScoringRuleCaseDao {
    @Override
    ...
    public List<ScoringRuleCase> getRuleCaseList(ScoringRule scoringRule) {
        List<ScoringRuleCase> ruleCaseList = getHibernateTemplate().
                find("SELECT s FROM ScoringRuleCase s WHERE s.rule= ?", scoringRule);
        return ruleCaseList;
    }
}
